...
from ui.modules.QueueItem import QueueItem
from ui.ui_files.QueueUI import Ui_queue_ui
import time
...
class QueueWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent: QtWidgets.QWidget = None) -> None:
        ...
        self.widget.left_arrow.setIcon(QtGui.QIcon(os.path.join("ui","icons", "chevron-left.svg")))
        self.widget.left_arrow.clicked.connect(lambda: self.change_position(True))
        self.widget.right_arrow.setIcon(QtGui.QIcon(os.path.join("ui","icons", "chevron-right.svg")))
    ...
    def remove_from_queue(self) -> None:
        if not self.selected:
            return
        if os.path.exists(os.path.join("ui","runtime_store", f"{self.selected.queue_file}.toml")):
            os.remove(os.path.join("ui","runtime_store", f"{self.selected.queue_file}.toml"))
