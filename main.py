...
class YoloPredictor(BasePredictor, QObject):
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None): 
        ...
        self.out_model_name = None       # 外部模型名称   
        self.source = ''                 # 输入源str
    ...
    def run(self):
        #try:
            ...
            if self.continue_dtc:  #暂停与继续的切换
                ...
                model = YOLO(self.new_model_name)#初始化模型
                ...
                while True:
                    try:
                        if self.continue_dtc:
                            ...
                            class_num_arr = []#检测到的类别统计
                            detections = sv.Detections.from_yolov8(result)
                            for each in detections.class_id:
                                if each not in class_num_arr:#如果不在类别库中
                                    class_num_arr.append(each)
                            class_num = len(class_num_arr)#类别数量
                            try:
                            ...
                            if self.save_txt:
                                with open('labels/result.txt','a') as f:
                                    f.write('当前时刻屏幕信息:'+str(labels2)+f'检测时间: {datetime.datetime.now().strftime("%Y-%m-%d-%H:%M:%S")}'+f' 路段通过的目标总数: {sum_of_count}')
                                    f.write('\n')
                            ...
                            if self.save_res and flag_save_video:
                                out = cv2.VideoWriter(f'pred_result/video_result_{video_id_count}.avi', fourcc,25, (width,height),True) #保存检测视频的路径
                                flag_save_video = 0
                            if self.stop_dtc:
                                if self.save_res:
                                    out.release()
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # Yolo-v8 thread 实例化之后进行参数配置-》再将实例加入线程
        ...
        self.main2yolo_begin_sgl.connect(self.yolo_predict.run)#线程检测任务的启动取决于 main2yolo_begin_sgl这个按键的触发
        self.yolo_thread = QThread() #启动一个线程    
        ...
        self.src_rtsp_button.clicked.connect(self.rtsp_seletction) #点击src_rtsp_button 将会触发self.rtsp_seletction事件
    ...
    def open_model_file(self):
        ...
        if not os.path.exists(open_fold):
            open_fold = os.getcwd()
        name, _ = QFileDialog.getOpenFileName(self, 'model', open_fold, "Pic File(*.pt *.engine)")
        if name:
            self.out_model_name = name
            self.show_status('手动添加外部模型：{}'.format(os.path.basename(name))) 
            config['open_fold'] = os.path.dirname(name)
            config_json = json.dumps(config, ensure_ascii=False, indent=2)  
            with open(config_file, 'w', encoding='utf-8') as f:
                f.write(config_json)
            mymodel = torch.load(self.out_model_name)
            torch.save(mymodel,'./weights/'+os.path.basename(name))
            self.stop()              
    #选择摄像头
    ...
    def show_traffic_graph(self):#加载流量折线图（屎山XD）
        if self.yolo_predict.run_started:
            if self.yolo_predict.show_graph is True and not self.yolo_predict.X_quit:
                ...
                MessageBox(
                    self.close_button, title='提示', text='正在停止继续加载车流量折线图...', time=500, auto=True).exec()
                return 
            MessageBox(
                    self.close_button, title='提示', text='正在加载车流量折线图...', time=500, auto=True).exec()
            ...
            MessageBox(
                    self.close_button, title='提示', text='请先开始检测...', time=500, auto=True).exec()
    ...
    def load_config(self):
        ...
        self.run_button.setChecked(False)  
        self.show_status("欢迎使用VisonWave-ML平台(工业级)")
    ...
    def change_model(self,x):
        ...
        self.show_status('更改模型为：%s' % self.select_model)
        self.Model_name.setText(self.select_model)
    #循环监测文件夹的文件变化
